...
class LeNet_Parabolic(nn.Module):
    def __init__(self, n_channels, n_classes, device, ks=3, fc_in=800, initial_scale=1.0):
        super(LeNet_Parabolic, self).__init__()
        ...
        self.pool1 = SemiPool2dParabolic(semifield=semifield1, c_in=20, c_out=20, kernel_size=ks, stride=2, device=device, initial_scale=initial_scale, padding='same')

        self.conv2 = nn.Conv2d(in_channels=20, out_channels=50, kernel_size=(5,5))
        self.relu2 = nn.ReLU()
        semifield2 = (self._maxvalues, torch.add, -1.0 * torch.inf, 0)
        self.pool2 = SemiPool2dParabolic(semifield=semifield2, c_in=50, c_out=50, kernel_size=ks, stride=2, device=device, initial_scale=initial_scale, padding='same')
