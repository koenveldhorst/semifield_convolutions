...
class cifarNet(nn.Module):
    def __init__(self, scale=3.0):
        ...
        if PARABOLIC:
            self.pool1 = SemiPool2dParabolic(SEMIFIELD, 64, 64, 5, 2, device, initial_scale=scale, padding='same')
            self.pool2 = SemiPool2dParabolic(SEMIFIELD, 128, 128, 5, 2, device, initial_scale=scale, padding='same')
            self.pool3 = SemiPool2dParabolic(SEMIFIELD, 256, 256, 5, 2, device, initial_scale=scale, padding='same')
        else:
...
def main(train=True, save=False, parabolic=False):
    ...
    for i in range(runs):
        ...
        model = cifarNet(scale=1.0).to(device)
        optimizer = torch.optim.SGD(model.parameters(), lr=0.01)
        criterion = nn.CrossEntropyLoss()

        if parabolic:
            model_name = f'cifar10_models/cifar_model_parabolic_{i}.pt'
        else:
            model_name = f'cifar10_models/cifar_model_{i}.pt'
    ...
    if parabolic:
        with open('cifar_classification_parabolic.json', 'w', encoding='utf-8') as f:
            json.dump(data, f, ensure_ascii=False, indent=4)
    else:
        with open('cifar_classification.json', 'w', encoding='utf-8') as f:
            json.dump(data, f, ensure_ascii=False, indent=4)
